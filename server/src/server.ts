...
app.get("/api/v1/getPublishers", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const err: Error = error as Error;
    result = assembleResultObject(false, "getPublishers: " + err.message, []);
    res.send(JSON.stringify(result));
  }
});
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      result = assembleResultObject(true, "getUpdatesForPublished", [updates]);
      res.send(JSON.stringify(result));
    } catch (error) {
      const err: Error = error as Error;
      result = assembleResultObject(
...
app.post("/api/v1/updatePublished", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const err: Error = error as Error;
    result = assembleResultObject(false, "updatePublished " + err.message, []);
  }
});
