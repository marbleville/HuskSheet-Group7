...
class Evaluator {
	...
	evaluate(node: ExpressionNode): string {
		if (node instanceof FunctionCallNode) {
			if (
				...
				return this.sum(
					this.rangeOp(node.expressions.at(0))
				).toString();
			} else {
	...
	private applyOp(op: string, left: string, right: string): number | string {
		switch (op) {
			...
			case "&":
				return (this.toNumber(left) & this.toNumber(right)) !== 0
					? 1
					: 0;
			case "|":
				return (this.toNumber(left) | this.toNumber(right)) === 1
					? 1
					: 0;
			default:
	...
	columnLetterToNumber(column: string): number {
		...
		for (let i = 0; i < column.length; i++) {
			number =
				number * 26 + (column.charCodeAt(i) - "A".charCodeAt(0) + 1);
		}
		return number;
	}
